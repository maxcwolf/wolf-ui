...
const STORIES_LIST_FILE = path.join('.storybook-web', 'storiesList.js')
...
const getStoriesListForWeb = (filePaths: string[]) =>
  filePaths
    .map((file) => "  '" + path.join('../storybook/', file) + "',")
    .join('\n')
...
const writeStoriesListForWebFile = (files: string[]) => {
  ...
  const content = storiesListForWebTemplate(
    getStoriesListForWeb(filesWithRelativePath),
  )
